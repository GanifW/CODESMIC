...
export function ChallengeEditorPage() {
  ...
  const runTest = () => {
    if (!question) return;
    setTestStatus('Memeriksa kode...');
    setSubmitStatus(null);

    const expected = question.answer?.trim() ?? '';
    if (!expected) {
      setTestStatus('❌ Tidak ada jawaban referensi untuk soal ini.');
      return;
    }
    ...
    setTestStatus(isCorrect ? '✅ Test berhasil.' : '❌ Jawaban belum benar.');
  };
  ...
  const handleSubmit = async () => {
    ...
    if (!token) {
      setSubmitStatus('❌ Silakan login terlebih dahulu untuk submit jawaban.');
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        setSubmitStatus(`❌ ${data.message || 'Jawaban salah.'}`);
      }
    } catch (err: any) {
      setSubmitStatus('❌ Gagal submit.');
    } finally {
      setIsSubmitting(false);
    }
  };
              ...
              <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                ...
                <div className="xl:col-span-2 space-y-4">
                  ...
                  >
                    ...
                    <button
                      className="px-8 py-4 bg-emerald-600 hover:bg-emerald-700 rounded-lg font-semibold text-lg shadow-lg shadow-emerald-500/50 hover:shadow-emerald-500/70 transition-all flex items-center justify-center gap-2 group disabled:opacity-60 disabled:cursor-not-allowed"
